classdef ARobotarium < handle
    ...
    properties (GetAccess = protected, SetAccess = protected)
        ...
        mat_file_path
        boundary_patch
        boundary_points = {[-4.0, 4, 4, -4.0], [-4, -4, 4.0, 4.0]};%{[-3, 3, 3, -3], [-2, -2, 2, 2]}; %
    end
    ...
    properties (GetAccess = public, SetAccess = protected)
        ...
        ghost_radius_flag = false
        ghost_box_flag = false
        label_flag = true
        boundary_flag = 'off'
        ghost_flag = true
        replay_flag = false
        ...
        % Arena parameters
        boundaries = [-3.5, 3.5, -3.5, 3.5];%[-3 3 -2 2]; %
    end
        ...
        %for i = 1:N
          %      if(abs(vs(1, i)) > (this.maxLinearVelocity/i))
               ...
               % if(abs(vs(2, i)) > (this.maxAngularVelocity/i))
                %   vs(2, i) = (this.maxAngularVelocity/i)*sign(vs(2, i));
              % end
           % end
          %  this.velocities(:, ids) = vs;
       % end
